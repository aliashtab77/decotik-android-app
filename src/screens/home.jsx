...
const Home = () => {
  ...
  const formatmoneystring = money => {
    return FormatWithTrailingISO('USD', 'en-US', money).trim();
  };
  ...
  const handlepdf = () => {
    let head = `<head><style>td${'{border:1px solid black;}'}</style></head><body><table><thead<tr><td>مصالح</td><td>متراژ مصرفی</td><td>قیمت${b}</td></tr></thead><tbody>`;
    tablets.map(
      (item, index) =>
        (head += `<tr><td>${item.name}</td><td>${
          item.meghdar
        }</td><td>${formatmoneystring(String(item.price))}</td></tr>`),
    );

    head += `<tr><td>قیمت نهایی</td><td>${formatmoneystring(
      String(fibalPrice),
      ...
      <View style={styles.container}>
            ...
            <DataTable>
              ...
              {tablets.map((item, index) => (
                <DataTable.Row key={index}>
                  ...
                  <DataTable.Cell style={styles.center} numeric>
                    {formatmoneystring(String(item.price))}
                  </DataTable.Cell>
                </DataTable.Row>
              ))}
            </DataTable>
            <Text style={styles.finalprice}>
              مجموع هزینه ها {formatmoneystring(String(fibalPrice))}
            </Text>
            <Ripple style={styles.btn} onPress={handlepdf}>
              <Text>دریافت به صورت PDF</Text>
            </Ripple>
          </>
        ) : (
          <Text />
        ...
        {tabletsd != null && saze == 'divar' ? (
          <>
            ...
            <Ripple style={styles.btn} onPress={handlepdf1}>
              <Text>دریافت به صورت PDF</Text>
            </Ripple>
          </>
        ) : (
